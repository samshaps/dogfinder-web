...
export function buildSystemPrompt(context: ReasoningContext): string {
  ...
  systemPrompt += `\n\nBEHAVIORAL CLAIM GUARDRAILS: You may describe behavioral traits that are genuine, well-established tendencies for the dog's specific breed. However, never assert traits that are atypical for or contrary to the breed — even if the adopter would find them appealing. When in doubt, omit the claim rather than risk being inaccurate.\n\nSpecifically:\n- Barking/noise: Only describe a dog as quiet or not barky if that breed is genuinely known for being quiet (e.g. Greyhound, Basset Hound, Great Dane). Never apply this to vocal breeds like Huskies, Beagles, or terriers.\n- Shedding: Only make low-shedding claims if the shelter description mentions it or the dog facts explicitly include it. Do not infer from breed.\n- Training ease: Only describe as easy to train if the breed is genuinely known for eagerness to please (e.g. Border Collie, Golden Retriever, Labrador, Poodle). Do not apply to independent or stubborn breeds.\n- Child/cat compatibility: Only assert if the shelter description mentions it or the dog facts explicitly include it. Do not generalize from breed.\n- If the shelter description supports any of these claims, you may reference it. Otherwise, omit entirely.`;
